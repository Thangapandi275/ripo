...
$.fn.EZView = function (collectionName) {
        ...
        if (!$('#EZView' + collectionName).length) {
            ...
            var tools =
                ...
                template = '<div id="' + collectionName + '" ' +
                    'class="EZView hide" style="display: flex; align-items: center; justify-content: center">' +
                    container + '</div>';
    ...
    self.buildHtmlContent = function () {
        ...
        if (isPdf) {
            ...
            var fileExtension = ['doc', 'docx', 'ppt', 'pptx', 'xls', 'xlsx'];
            if ($.inArray(src.split('.').pop().toLowerCase(), fileExtension) == -1) {
            }
            else {
                var locationpath = "http://demo.softwarecubes.com/mmsdemo/";
                //console.log(locationpath);
                var tempSrc = src.replace('../', '');
                //console.log(tempSrc);
                //
                var path = "https://view.officeapps.live.com/op/embed.aspx?src=" + locationpath + "/" + tempSrc;
                //console.log(path);
                htmlContent = '<iframe src="' + path + '"  width="' + $(window).width() * 0.9 + '" height="' + $(window).height() * 0.95 + '" frameborder=0>This is an embedded</iframe>';
                //htmlContent = '<iframe src="https://view.officeapps.live.com/op/embed.aspx?src=https://tmmuae.com/Repositary/Images/Registration/Documents/2.pptx"  width="' + $(window).width() * 0.9 + '" height="' + $(window).height() * 0.95 + '" frameborder=0>This is an embedded</iframe>';
                //self.arContent[collectionName][imgIndex].isImg = false;
            }
        }
        return htmlContent;

...
(function ($, window, document, undefined) {
    ...
    $.fn.imageuploadify = function (opts) {
        ...
        this.each(function () {
            ...
            const retrieveFiles = (files) => {
                ...
                for (let index = 0; index < files.length; ++index) {
                    if (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
                        ...
                        readingFile(id, files[index], index);
                        totalFiles.push({
                            id: id,
                            file: files[index]
                        });
            ...
            const readingFile = (id, file, index) => {
               
                const fReader = new FileReader();
                const width = dragbox.width();
                const boxesNb = Math.floor(width / 100);
                const marginSize = Math.floor((width - (boxesNb * 100)) / (boxesNb + 1));
                let container = $(`<div class='imageuploadify-container' id='containerclear'><button id='btnclear' type='button' class='btn btn-danger btn-clear'><span class="fa fa-times del-img"></span></button><div class='imageuploadify-details'><span>${file.name}</span><span>${file.type}</span><span>${file.size}</span></div></div>`);
                let details = container.find(".imageuploadify-details");
                container.find("#btnclear").attr('id', 'btnclear' + (index + 1));
                container.attr('id', 'containerclear' + (index + 1));
                ...
                if (file.type && file.type.search(/image/) != -1) {
                    ...
                    let type = "<i class='mdi mdi-file' style='display:block;color:#3e4b5b;font-size:22px;margin: 15px 15px 15px 0;'></i>";
                    if (file.type == "application/pdf") {
                        type = "<i class='mdi mdi-file-pdf' style='display:block;color:#3e4b5b;font-size:22px;margin: 15px 15px 15px 0;'></i>";
                    }
                    else {
                        type = "<i class='mdi mdi-file-word' style='display:block;color:#3e4b5b;font-size:22px;margin: 15px 15px 15px 0;'></i>";
                    }
                    ...
                    fReader.onloadend = function (e) {
                        ...
                        let divisPdf = $("<span>" + type + "</span>");
                        //span.css("font-size", "5em");
                        container.append(divisPdf);
                        imagesList.append(container);
                        imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+4)").css("margin-left", marginSize + "px");
                        imagesList.find(".imageuploadify-container:nth-child(" + boxesNb + "n+3)").css("margin-right", marginSize + "px");
                    };
            ...
            dragbox.on("drop", function onDrop(event) {
                ...
                $.toast({
                    heading: 'Warning',
                    text: 'Please upload files, drag and drop will not support',

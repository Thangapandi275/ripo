...
class Common extends MY_Model{
    ...
    public function getCategories(){
       ...
       $this->db->select('categoryname,cr_category.isActive,cr_category.IsDelete,branch_name,branch_id,category_branch_id,category_id')->from('cr_category')->where('cr_category.IsDelete',0);
       if($sessionData['branch'] != 0){
        ...
        $this->db->order_by('cr_category.category_id',"DESC");      
    ...
    public function getproducts(){
        $sessionData = $this->session->userdata();
         $this->db->select('id,product_id,product_branch_id,product_name,purchase_amount,rent_amount,instock,outstock,cr_products.isActive,cr_products.IsDelete,categoryname,branch_name,pro_image')->from('cr_products')->where('cr_products.IsDelete',0);
         if($sessionData['branch'] != 0){
    ...
    public function getusers(){
        $sessionData = $this->session->userdata();
         $this->db->select('branch_name,usertype,user_id,username,cr_users.isActive,fullname,contact')->from('cr_users')->where('cr_users.IsDelete',0);
         if($sessionData['branch'] != 0){
          ...
          $this->db->join('cr_usertype','cr_usertype.usertype_id = cr_users.usertype_ref_id','left');    
           $this->db->order_by('cr_users.user_id',"DESC");          
         $data = $this->db->get()->result();
        return $data;
    }
    public function getcustomers(){
        $sessionData = $this->session->userdata();
         ...
         if($sessionData['branch'] != 0){
          ...
          $this->db->order_by('cr_customer.customer_id',"DESC");       
         $data = $this->db->get()->result();
        return $data;
    }

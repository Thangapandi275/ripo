...
//        $config['max_height'] = '768';
    ...
    public function my_permission($table, $user_id)
    {
		...
		// echo $this->db->last_query();
        if (!empty($result_info)) {
            foreach (array_reverse($result_info) as $result) {
                if ($result->permission == 'all') {
                    $permission[] = $result;
                } else {
                    $get_permission = json_decode($result->permission);
    ...
    function generate_invoice_number()
    {
	 ...
	 public
    function generate_quotations_number()
    {
        $query = $this->db->select('reference_no')->select_max('quotations_id')->get('tbl_quotations_workorder');
        if ($query->num_rows() > 0) {
            $row = $query->row();
            $ref_number = intval(substr($row->reference_no, -4));
            $next_number = ++$row->quotations_id;
            if ($next_number < $ref_number) {
                $next_number = $ref_number + 1;
            }
            if ($next_number < config_item('invoice_start_no')) {
            ...
            $next_number = $this->reference_no_exists_quotation($next_number);
            $next_number = sprintf('%04d', $next_number);
            return $next_number;
        } else {
            return sprintf('%04d', config_item('invoice_start_no'));
	...
	public
    function reference_no_exists_quotation($next_number)
    {
        $next_number = sprintf('%04d', $next_number);

        $records = $this->db->where('reference_no', config_item('quotation_prefix') . $next_number)->get('tbl_quotations_workorder')->num_rows();
        if ($records > 0) {
    ...
    function send_email($params)
    {
        ...
        if (config_item('use_postmark') == 'TRUE') {
           ...
           /* if (config_item('protocol') == 'smtp') {
            ...
            // Send email
            $config['useragent'] =  DEVELOPED_BY;
            $config['mailtype'] = "html";
            $config['newline'] = "\r\n";
            $config['charset'] = 'utf-8';
            $config['wordwrap'] = TRUE;
 			$config['mailpath'] = "C:\Program Files (x86)\Plesk\Additional\PleskPHP56\pear\Mail.php";
    ...
    {
        $all_grade = $this->db->get('tbl_grade')->result();
      //  print_r($all_grade);
            return $all_grade;
     }
